                    ...
                    if controls.active || startsAutomatically {
                        ...
                        ScrollView {
                            StartCallCard(contact: $request.endUserContact, enrollment: controls.enrollment,
                                          ...
                                          requiresContact: startCall == nil, starting: controls.starting) {
                                if let startCall {
                                    Task { await controls.start(startCall) }
                                    return
                                }
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            VStack(spacing: 12) {
                ...
                Button(action: start) {
                    Label(starting ? "Starting call..." : "Start call", systemImage: "phone").font(.system(size: 14, weight: .medium))
                ...
                .disabled(starting || enrollmentBusy || (requiresContact && contact.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty))
            }
        }
