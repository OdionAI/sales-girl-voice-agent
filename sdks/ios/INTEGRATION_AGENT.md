...
For a compatible release of the **new key-based API**, use the instructions in
[ACCOUNT_KEYS.md](ACCOUNT_KEYS.md) instead. That path needs only a publishable key
and agent ID, plus an optional authenticated caller-token provider. Do not ask
for an endpoint/business slug or build a `SampleModel` for it. It is not yet in
staging.2; the handoff below is retained for that released advanced API.
