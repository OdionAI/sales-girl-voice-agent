...
public class Room: NSObject, @unchecked Sendable, ObservableObject, Loggable {
    ...
    var _blockProcessQueue = DispatchQueue(label: "AttentiveRTCSDK.engine.pendingBlocks",
                                           qos: .default)
                ...
                roomOptions: RoomOptions? = nil)
    {
        ...
        super.init()
        // log sdk & os versions
        log("sdk: \(AttentiveRTCSDK.version), ffi: \(AttentiveRTCSDK.ffiVersion), os: \(String(describing: Utils.os()))(\(Utils.osVersionString())), modelId: \(String(describing: Utils.modelIdentifier() ?? "unknown"))")
