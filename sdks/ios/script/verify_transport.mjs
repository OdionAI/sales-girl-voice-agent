...
assert.deepEqual(readJSON(path.join(sdk, 'Package.resolved')).pins.map(pin =>
  [pin.identity, pin.state.version, pin.state.revision]),
  [['swift-protobuf', pins.protobuf.version, pins.protobuf.revision]], 'Unexpected source package dependency');

// The diagnostic app consumes our published binary, not the internal source package.
const sample = path.join(sdk, 'Examples/AttentiveSample/AttentiveSample.xcodeproj');
const project = JSON.parse(execFileSync('plutil', ['-convert', 'json', '-o', '-', path.join(sample, 'project.pbxproj')],
  { encoding: 'utf8' }));
const packages = Object.values(project.objects).filter(value => value.isa === 'XCRemoteSwiftPackageReference');
assert.equal(packages.length, 1);
assert.equal(packages[0].repositoryURL, 'https://github.com/OdionAI/attentive-ios-sdk.git');
assert.equal(packages[0].requirement.kind, 'exactVersion');
const samplePins = readJSON(path.join(sample, 'project.xcworkspace/xcshareddata/swiftpm/Package.resolved')).pins;
assert.deepEqual(samplePins.map(pin => [pin.identity, pin.state.version]),
  [['attentive-ios-sdk', packages[0].requirement.version]], 'Unexpected binary sample dependency');
assert.match(samplePins[0].state.revision, /^[a-f0-9]{40}$/);
